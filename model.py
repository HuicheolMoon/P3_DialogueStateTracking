...
class TRADE(nn.Module):
    ...
    def forward(
        ...
        encoder_outputs, pooled_output = self.encoder.forward(input_ids=input_ids)
        all_point_outputs, all_gate_outputs = self.decoder.forward(
            input_ids,
            encoder_outputs,
    ...
    def forward(
        ...
        for k in range(max_len):
            ...
            if k == 0:
                ...
                all_gates_outputs = gated_logit.view(batch_size, J, self.n_gate)
            all_point_outputs[:, :, k, :] = p_final.view(batch_size, J, self.vocab_size)
        return all_point_outputs, all_gates_outputs
